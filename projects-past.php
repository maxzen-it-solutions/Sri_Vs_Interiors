...
<head>
  <meta charset="utf-8">
  <title>Sri VS Interiors | Past Projects</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">

  <!-- Use header.php for CSS/JS -->
</head>
<body>
...
<div class="content-wrapper">
  ...
  $products = [];
  $stmt = $conn->prepare("SELECT id, name, client_name, address, description, category, created_at FROM products WHERE status='active' AND LOWER(category)='past' ORDER BY created_at DESC");
  $stmt->execute();
  $res = $stmt->get_result();
  ...
  if (empty($products)) {
      echo '<section style="padding:40px 0;"><div style="max-width:1200px;margin:0 auto;">No past projects found.</div></section>';
  } else {
      // Default to first project
      $currentProject = $products[0];
      $pid = $currentProject['id'];
      $images = [];
      $imgStmt = $conn->prepare("SELECT id, image_path FROM project_images WHERE project_id = ? ORDER BY id ASC");
      if ($imgStmt) {
  ...
  ?>
      ...
      <section id="main-project" aria-label="past-projects" style="padding:50px 0; font-family:Arial,sans-serif;">
        <div style="max-width:1200px; margin:0 auto; display:flex; flex-wrap:wrap; gap:30px;">
          <div style="flex:2; min-width:300px;">
            <div style="border-radius:10px; overflow:hidden; box-shadow:0 5px 20px rgba(0,0,0,0.1);">
              <img id="mainProjectImg" src="<?php echo htmlspecialchars($mainImg); ?>" alt="<?php echo htmlspecialchars($currentProject['name']); ?>" 
                        style="width:100%; height:70vh; object-fit:cover; display:block; border-radius:10px;">
            </div>
            <?php if (!empty($images)): ?>
            <div style="display:flex; justify-content:center; gap:10px; margin-top:10px;">
              <?php foreach ($images as $img): ?>
                <img src="<?php echo htmlspecialchars($img); ?>" onclick="changeMainImage(this)" style="width:60px; height:60px; object-fit:cover; border-radius:5px; cursor:pointer;">
              <?php endforeach; ?>
            </div>
            <?php endif; ?>
          </div>
          ...
          <div style="flex:1; min-width:250px;">
            <div style="background:#fff; padding:25px; border-radius:10px; box-shadow:0 5px 20px rgba(0,0,0,0.05);">
              <h2 style="margin-top:0; color:#333;"><?php echo htmlspecialchars($currentProject['name']); ?></h2>
              <p style="color:#666; line-height:1.6;"><?php echo nl2br(htmlspecialchars(substr($currentProject['description'],0,1000))); ?></p>
              <div style="margin-top:20px;">
                ...
                <p><strong>Address:</strong> <?php echo htmlspecialchars($currentProject['address']); ?></p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="scroll-item" onclick="loadProject(<?php echo $pid; ?>)">
                <div class="hovereffect">
                  ...
                  <div class="overlay">
                    ...
                    <h3><?php echo htmlspecialchars($p['address']); ?></h3>
                  </div>
                </div>
              </div>
            <?php endforeach;
