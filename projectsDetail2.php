...
<head>
  <meta charset="utf-8">
  <title>Sri VS Interiors | Present Projects</title>
...
<?php
// Fetch all present projects
$projects = [];
$stmt = $conn->prepare("SELECT id, name, client_name, address, description, category, created_at FROM products WHERE status='active' AND LOWER(category)='present' ORDER BY created_at DESC");
$stmt->execute();
$res = $stmt->get_result();
...
?>
  ...
  <section id="main-project" aria-label="present-projects" style="padding:50px 0; font-family:Arial,sans-serif;">
      ...
      <div style="flex:2; min-width:300px;">
        ...
        <div style="display:flex; justify-content:center; gap:10px; margin-top:10px;">
          <?php foreach ($images as $img): ?>
            <img src="<?php echo htmlspecialchars($img); ?>" alt="" onclick="document.getElementById('mainProjectImg').src=this.src" style="width:60px; height:60px; object-fit:cover; border-radius:5px; cursor:pointer;">
          <?php endforeach; ?>
        </div>
        <?php endif; ?>
      </div>
        ...
        <div class="scroll-item" onclick="loadProject(<?php echo $pid; ?>)">
          <div class="hovereffect">
            ...
            <div class="overlay">
              ...
              <p><?php echo htmlspecialchars($p['address']); ?></p>
            </div>
          </div>
        </div>
        <?php endforeach;
